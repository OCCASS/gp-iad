...
- `LinearSVC` + TF-IDF
- `RandomForestClassifier` или `CatBoostClassifier` — сравнение с ансамблевыми подходами.
...
### 6.2. Метрики для классификации сообщений

| Метрика              | Зачем нужна                                            |
| -------------------- | ------------------------------------------------------ |
| Accuracy             | общая доля верных классификаций                        |
| Macro F1-score       | качество с учетом всех классов, включая редкие         |
| Weighted F1-score    | качество с учетом частоты классов                      |
| Precision по классам | насколько точно бот отправляет нужные типы уведомлений |
| Recall по классам    | насколько редко бот пропускает важные события          |
| Confusion matrix     | анализ ошибок между похожими классами                  |
